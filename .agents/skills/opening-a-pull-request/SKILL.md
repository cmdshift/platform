...
- All commits on the feature branch (`feat/<topic>` / `fix/<topic>`), never `main`; pushed with the human's approval.
- Docs made stale by the change are updated **in the same branch** — a change isn't PR-ready until the docs it invalidated are in it (AGENTS.md → "Docs maintenance").
