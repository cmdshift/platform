...
3. `just cluster apply -auto-approve` (not health-gated — returns after the machine-config applies + bootstrap, and the bootstrap API gate in step 4 owns readiness; the former `talos_cluster_health` node-health gate was removed as redundant, cmdshift/platform#73). **If it hangs at `talos_machine_bootstrap`**: root cause is a stale host port binding after container churn — the published path accepts but black-holes (first root-caused on the historical macOS/Docker Desktop host's VM publisher; shape documented in the runbook). Recovery: kill the apply, `docker restart $(docker ps -q --filter name=cmd-local-test)` (an LB restart — the ctrl nodes keep running), re-run the apply — only bootstrap + kubeconfig remain and they land in seconds. The provider now fails fast (10s timeouts) instead of its old silent 10m retry. Verify with `curl -skf --max-time 3 https://127.0.0.1:6443/version` — 401 = binding alive; TLS `SSL_ERROR_SYSCALL` right after a restart = publisher alive, API still booting (~60s), NOT a stale binding (a stale binding is a silent hang with no TLS stage) — don't restart twice. Non-interactive shells MUST pass `-auto-approve` (the approval prompt EOFs otherwise).
