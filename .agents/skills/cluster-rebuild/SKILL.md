...
3. `just cluster apply -auto-approve` — **can hang**: spawn it in the background, kill it after ~1 minute (kill the `just` PID + `pkill -f "chdir=cluster/local apply"`), wait **20-30s** before bootstrapping. It may also finish on its own in ~16s — poll before killing. Non-interactive shells MUST pass `-auto-approve` (the approval prompt EOFs otherwise) and macOS has no `setsid` (background + `kill $PID` + pkill instead).
