...
- **build fails locally but flux builds fine** — a dir without a `kustomization.yaml` (e.g. `crds/`) gets an **implicit kustomization auto-generated by kustomize-controller** listing all YAMLs, so standalone `kustomize build`/`kubectl kustomize` on it fails while flux applies it happily (cmdshift/platform#120). Verify with the same engine: `flux build kustomization <name> --path <dir>`. Full trap: [runbooks/local/namespace-migration.md](../../../runbooks/local/namespace-migration.md).
