...
Commit/push by **permission** (AGENTS.md Hygiene). When reconcile is green and docs are swept, summarize the change and propose a commit — then **ask the human before running `git commit`/`git push`**; the human reviews the diff and approves history. The commit step follows the `making-a-commit` skill (conventional format, signing rule); the PR step follows `opening-a-pull-request`.
