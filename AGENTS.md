...
The standard loop (`platform-workflow`): rationale-comment check → `yaml_lint` → `helm_verify` → `sync_wait` → `flux_wait` (reconcile from the root) → final checks (`kubectl get helmreleases -A` green, `policy_report` failures 0, `nsa_scan` baseline **92.3, zero failing controls** — C-0069/C-0070 `notEvaluated` is the lean ceiling, not a regression).
