...
Documentation is part of the change — a change isn't ready to commit or PR until the docs it made stale are updated in the same branch. Sweep the surfaces:

- **`CHANGELOG.md`** — dated learnings, incident narratives, open follow-ups (append a reverse-chron entry; ref `cmdshift/platform#N`)
- **group READMEs** (`manifests/local/<group>/README.md`, + `manifests/local/README.md` for cross-cutting conventions and the hardening-deviations baseline; `manifests/cloud/notes.md` for cloud deltas) — group-level decisions the change touched, kept current
- **runbooks** (`runbooks/local/`, incl. `incidents.md`) — procedures that changed, new gotchas, incident post-mortems
...
Helper scripts for the repeated plumbing (direnv adds `tools/bin` to PATH; invoke as `tools/bin/<name>` otherwise). direnv also exports `KUBECONFIG`/`TALOSCONFIG` (→ `cluster/local/.tmp/`): `kubectl` and `talosctl` work bare in a direnv shell — no explicit config flags or `KUBECONFIG=` prefixes (node-targeted talosctl commands like `read` still need `-n <node-ip>`; the context sets no default node). **Check here before formulating any kubectl/jq/prometheus/docker command by hand** — audits (`memory_audit`/`cpu_audit`/`request_audit`/`vpa_recs`/`policy_report`), observability queries (`prometheus_query`/`loki_query`/`mailpit`), and waits (`flux_wait`/`helm_wait`/`velero_wait`) already exist and handle the plumbing you'd get wrong inline: port-forward lifecycle, Mi/Gi/m normalization, bounded polling. When a task needs more than a round or two of throwaway plumbing anyway, promote it to a new script here instead of re-deriving it next time (the proven pattern: `policy_report`/`cpu_audit`/`kyverno_unblock` all started as inline jq — see cmdshift/platform#21).

For multi-minute waits (`flux_wait`/`helm_wait`/`velero_wait`/`bench`, image pulls, reconciles), don't block the session on sleep loops: launch the command in the background (or dispatch a background subagent with a bounded poll) and do other work meanwhile — and when something fails early, diagnose immediately (StartError, admission denial, failed mounts at t=10s) instead of polling blind until a timeout.

Full reference — what each does, arguments, defaults, exit codes, gotchas: [tools/bin/README.md](tools/bin/README.md). One-line map: `yaml_lint` (manifest lint), `helm_verify` (HelmRelease values render check), `cr_validate` (server-side dry-run of CRs against on-cluster CRD schemas + admission — run before `sync_wait` on any CR change), `sync_wait` (bucket-sync convergence before reconciling), `flux_wait` (reconcile root + bounded poll), `helm_wait` (reconcile one HelmRelease; exits fast with the failure message when the release is terminally broken), `memory_audit`/`cpu_audit` (usage-vs-**limits**), `request_audit` (usage-vs-**requests**, the scheduling side), `vpa_recs` (VPA recommendations vs requests — the evidence side; goldilocks maintains the VPAs), `pod_status` (pod table with restarts/exit codes, crashloop triage), `policy_report` (admission results), `kyverno_unblock` (hostNetwork rollout deadlock), `prometheus_query`/`loki_query`/`mailpit` (observability), `velero_wait` (backup/restore poll), `rustfs` (rc CLI in the storage container), `cilium_test` (connectivity test + temp scaffolding), `bench` (episodic kube-bench CIS scan + temp scaffolding; triage ledger in `manifests/local/security/README.md`).
...
- **kyverno**: `admissionController.container.resources` is nested (unlike background/cleanup/reports); `config.webhooks` is a **map** (a list is silently dropped by helm merge); `backgroundScanInterval: 1h` (5m caused a reports-controller CPU ramp — cmdshift/platform#17); controllers use hostNetwork → rollouts deadlock on host ports — **any** controller bump including a resource-only values change (helm-controller updates the deployment spec but the old pods hold the host ports; hit with the reports-controller hanging `Unknown Running upgrade`, cmdshift/platform#66) — recover with `kyverno_unblock` (deletes the stale-generation **ReplicaSets** — pod deletion is whack-a-mole, the stale RS respawns and the deployment controller re-scales it; if the old release still lives in a former namespace, delete its deployments instead — the ns refactor (cmdshift/platform#31) hit the cross-ns variant). It runs in the `policies` ns and reads PolicyExceptions from there; PolicyException CEL updates can lag in the admission engine even after the generator logs them — bump the object (annotation via manifest) to force re-pick-up. Namespaces follow their kustomization group (operators in `certificates`/`secrets`/`policies`/`storage`/`objects`/`monitoring`/`backups`/`logging`/`security`; `kube-system`/`flux-system` excepted) — runbooks/local/namespace-migration.md
...
- **velero**: node-agents are privileged by design ("remove in the cloud" comments mark local-only settings). FSB skips hostPath PVs — the StorageClasses carry `defaultVolumeType: local` so **new** PVCs get `local` PVs and are volume-data protected (all 8 PVs converted with the rebuild that introduced the annotation). The temporary data mover pods (velero 1.15+ hosting-pod design) need the velero PolicyException + the `node-agent-config` configmap — the configmap **ships with the release in `backups/`**, not `backups-config/`: velero exits at startup if the flag's configmap is missing (the circular-wedge story is in the runbook). Delete backups with `velero backup delete`, never `kubectl delete backup` — backup-sync resurrects the Backup CR from storage within minutes. Operations: the `velero-ops` skill
- **cilium**: the chart is pinned in TWO places — the bootstrap helm_release (`cluster/local/bootstrap/main.tf`) and the flux HelmRelease (`networking/cilium.helm-release.yaml`) — and the two `version` pins must be bumped in lockstep, or the flux adoption converges the live release to whichever pin lags (the local tree carries `1.21.0-pre.2` for the Linux host's kernel 7.2, cilium/cilium#48016; see [manifests/cloud/notes.md](manifests/cloud/notes.md) for the cloud-side version rule)
- **thanos-operator**: deployed from its repo's `bundle.yaml` via kustomization, not its helm chart — the chart embeds ~2.5MB of CRDs in the release manifest, blowing helm's 1MB release-secret cap (rationale comment in `monitoring/thanos-operator.kustomization.yaml`; strategy ladder in `adopt-chart`). Status conditions are trustworthy since thanos-community/thanos-operator#636 (commit `831accd4`): a single `Ready` condition, failure-recoverable — the `monitoring-config` kustomization gates the four thanos CR kinds on it via `healthCheckExprs`. GitRepository commit and quay image tag (`main-YYYY-MM-DD-<shortsha>`) must be bumped in lockstep. **Ruler alerting depends on the query seeing the prometheus head** — the sidecar endpoint is wired manually via `additionalArgs` in `main.thanos-query.yaml` (label-based discovery can't see the chart-managed discovery service); if ruler rules silently never fire, check `prometheus_query --query 'count(kube_pod_container_status_restarts_total)'` against the query svc — empty means the head path is broken again. The store-path sibling hit live: a stale SRV-resolved store pod IP makes the query fail WHOLE requests (even head-only rules) until it re-resolves — ruler logs `no query API server reachable`, dial-test the store IP from the query pod (`wget http://<store-ip>:10902/-/ready`) before suspecting network policy; it is not a CNP block. Alert delivery lands in mailpit (http://mail.cloud.test)
- **AlertmanagerConfig** (`monitoring.coreos.com/v1alpha1`): child-route `matchers` are **structured Matcher objects** (`{name, value}`), not PromQL strings — strings fail the validating webhook with an opaque "invalid". Receiver names that look like YAML nulls (`name: null`) must be quoted (`name: "null"`) or the CRD rejects the object before the webhook even sees it. Both hit live in `monitoring-config/mail.alertmanager-config.yaml` (the quoted-null receiver went away with the kube-proxy TargetDown mute, cmdshift/platform#70 — the matcher rule stands)
