...
- **Keep docs in the change** — a change isn't ready to commit or PR until the docs it made stale are updated in the same branch (see Docs map below; dispatch heavy sweeps as a background `docs-sweep` subagent so the session isn't blocked).
...
- **Don't invoke anything longer than 60 seconds** — local ops either make progress or fail fast; no sleep loops, no blind polling to a long timeout. Long waits (`flux_wait`/`helm_wait`/`velero_wait`/`bench`, image pulls, rebuild polls) run in the background while work continues; diagnose early failures immediately instead of waiting out the timeout.
