...
- **Rationale comments** (issue #43): comment only **surprising decisions and settings** — not values that explain themselves or explicitly desired outcomes. **No dated comments**; provenance goes in issue/PR refs instead. **Don't explain how tools work** (flux propagation, kustomize patch mechanics, helm template internals) — link the runbook/notes entry that owns the story. Keep the evidence numbers (observed usage, P99) at the value — they're what justifies it; link notes/runbook/issue if there's more context.
...
- **thanos-operator**: deployed from its repo's `bundle.yaml` via kustomization, not its helm chart — the chart embeds ~2.5MB of CRDs in the release manifest, blowing helm's 1MB release-secret cap (rationale comment in `monitoring/thanos-operator.kustomization.yaml`; strategy ladder in `adopt-chart`). Status conditions are trustworthy since thanos-community/thanos-operator#636 (commit `831accd4`, 2026-09-07): a single `Ready` condition, failure-recoverable — the `monitoring-config` kustomization gates the four thanos CR kinds on it via `healthCheckExprs`. GitRepository commit and quay image tag (`main-YYYY-MM-DD-<shortsha>`) must be bumped in lockstep. **Ruler alerting depends on the query seeing the prometheus head** — the sidecar endpoint is wired manually via `additionalArgs` in `main.thanos-query.yaml` (label-based discovery can't see the chart-managed discovery service); if ruler rules silently never fire, check `prometheus_query --query 'count(kube_pod_container_status_restarts_total)'` against the query svc — empty means the head path is broken again. The store-path sibling hit live 2026-09-07: a stale SRV-resolved store pod IP makes the query fail WHOLE requests (even head-only rules) until it re-resolves — ruler logs `no query API server reachable`, dial-test the store IP from the query pod (`wget http://<store-ip>:10902/-/ready`) before suspecting network policy; it is not a CNP block. Alert delivery lands in mailpit (http://mail.cloud.test)
...
- **Docker Desktop port bindings go stale after rapid container churn** (destroy → recreate of a port-publishing container within ~a minute): the host listener still ACCEPTS connections but black-holes the forward into the VM — container and node stay healthy, only the host→container published path dies. Hit live on `cmd-local-test:50000` (issue #4): the talos provider silently retried the TLS handshake for its 10m default create timeout, looking exactly like an "indefinite bootstrap hang". Recovery: `docker restart cmd-local-test`, re-apply (only bootstrap + kubeconfig remain; seconds). The bootstrap/kubeconfig resources now carry 10s `timeouts` — a hang surfaces as a fast error, not a wedge. Diagnose with the haproxy stats socket (`docker exec cmd-local-test wget -qO- 'http://127.0.0.1:8404/stats;csv'` — apid frontend `stot` frozen while retries flood = binding dead). Full story in runbooks/local/cluster-rebuild.md; same failure shape as the companion `cloud-test` restart note in that runbook.
