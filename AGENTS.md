...
- **Docker Desktop port bindings go stale after rapid container churn** (destroy → recreate of a port-publishing container within ~a minute): the host listener still ACCEPTS connections but black-holes the forward into the VM — container and node stay healthy, only the host→container published path dies. Hit live on `cmd-local-test:50000` (cmdshift/platform#4): the talos provider silently retried the TLS handshake for its 10m default create timeout, looking exactly like an "indefinite bootstrap hang". Recovery: `docker restart cmd-local-test`, re-apply (only bootstrap + kubeconfig remain; seconds). The bootstrap/kubeconfig resources now carry 10s `timeouts` — a hang surfaces as a fast error, not a wedge. Diagnose with the haproxy stats socket (`docker exec cmd-local-test wget -qO- 'http://127.0.0.1:8404/stats;csv'` — apid frontend `stot` frozen while retries flood = binding dead). Full story in runbooks/local/cluster-rebuild.md; same failure shape as the companion `cloud-test` restart note in that runbook.
