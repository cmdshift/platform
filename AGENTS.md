...
- **Commits and pushes are human-only**: the agent's job ends at a green reconcile + docs swept — it stops there and hands back (proposing a commit split is fine; running `git commit`/`git push` is not). The human reviews the diff and makes history.
