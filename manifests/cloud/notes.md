...
Cloud deltas:
- **`alloy.configMap` wiring is load-bearing** — the grafana/alloy chart **silently installs its example config** (pods healthy, nothing pushed, zero errors) if the values block (`create: false` / `name: alloy-config` / `key: config.alloy`) is missing; locally this was the #27 ingestion outage (dropped by an edit that overwrote the block, cmdshift/platform#27). Verify the running config from day one with `alloy_components`: the expected set is `discovery.kubernetes.pods` + `loki.source.kubernetes.pods` + `loki.process.wrap` + `loki.write.endpoint` — `discovery.kubernetes.{nodes,services,endpoints,...}` appearing means the example config is live.
- **Stream labels**: streams carry `instance` (`ns/pod:container`), `job`, `service_name`, `detected_level` — **no `namespace`/`pod` labels** (the k8s metadata the pack stage produces is embedded in the log line). Log-based alert rules and dashboards in the cloud must select on labels that exist (or on the promoted labels if the label-pipeline follow-up lands locally first — see `manifests/local/notes.md`).
