...
- **Beyla `contextPropagation.enabled: true` is the chart DEFAULT and forces `hostNetwork: true` + cgroup/tracefs hostPath mounts** — hostNetwork makes the API server default hostPort=containerPort on the metrics port (kyverno disallow-host-ports denial; and the observability CNP only allows host egress 10250/9100, so the ServiceMonitor scrape would break anyway). Set `contextPropagation.enabled: false`: Beyla then mounts tracefs inside its own mount namespace — privileged container, no hostPath bind, no tetragon-style tracefs conflict. The `allow-beyla-ebpf` PolicyException (`policies-config/`) covers capabilities/host-namespace/host-path/privileged/run-as-non-root/seccomp; `disallow-host-ports` + `require-graceful-termination` are deliberately NOT exempted — beyla pods pass them legitimately.
- **Beyla config goes via chart values `config.data`** (otel_traces_export endpoint, log_format json) — not BEYLA_ env vars; the obi config binds `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT` but the configmap route is the clean path. Export shape: prometheus on 9090 (ServiceMonitor) + OTLP-traces direct to `http://tempo.observability.svc:4318` — no OTel collector in the path. The missing `OTEL_EBPF_KUBE_CLUSTER_NAME` warn is harmless (network metrics only).
- **Sizing evidence**: beyla 512Mi limit OOMKilled repeatedly on busy work nodes (exit 137, ~20-40s after start — eBPF ring buffers + k8s metadata cache) → 768Mi stable (steady 400-480Mi on all 4 work nodes, zero restarts since). Tempo lean start validated 268-367Mi steady (768Mi request / 1Gi limit), CPU 150m→1000m.
- **Beyla runs on the 4 work nodes only** — the ctrl-node taint is not tolerated by design (no user workloads there).
- **Grafana wiring**: `observability-config/tempo.grafana-datasource.yaml` (`GrafanaDatasource` CR, type `tempo`, url `http://tempo.observability.svc:3200`); the `platform-deployment` dashboard gained a `tempo_ds` datasource variable + a "Recent traces (Beyla)" traces panel (id 7, filters on `k8s.namespace.name=$namespace`).
