...
| thanos-operator | GitRepository commit (`sources/thanos-operator.git-repository.yaml`) + quay image tag (`monitoring/thanos-operator.kustomization.yaml` patch) | commit and quay `main-<date>-<sha>` tag move in lockstep |
