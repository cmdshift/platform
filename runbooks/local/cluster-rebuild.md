...
| OIDC kubeconfig | `kubectl --kubeconfig .tmp/kubeconfig-oidc get nodes` | `Error from server (Forbidden): User "https://auth.cloud.test/realms/platform#test" cannot list resource "nodes"` — **that Forbidden IS the success** (browser login via the kubelogin exec plugin worked, authn passed; RBAC is cmdshift/platform#91's scope). The exec plugin needs kubelogin ≥1.36 on the host (`aur/kubelogin` on Arch) and the root CA trusted (prerequisite row above) |
