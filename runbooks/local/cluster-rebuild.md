...
| OIDC kubeconfig | `kubectl --kubeconfig .tmp/kubeconfig-oidc get nodes` | Lists nodes (user `admin@cloud.test` is in rauthy group `platform-admin`, which the `access/` group's ClusterRoleBinding maps to `cluster-admin`, cmdshift/platform#91). The exec plugin needs kubelogin ≥1.36 on the host (`aur/kubelogin` on Arch) and the root CA trusted (prerequisite row above). Authorization check without a browser: `kubectl auth can-i list pods -n default --as=u --as-group=platform-view` → yes, `--as-group=platform-view ... get secrets` → no |
| OIDC viewer user | `kubectl --kubeconfig .tmp/kubeconfig-oidc get secrets -n default` after logging in as `viewer`/`viewer123` | **Forbidden** (rauthy group `platform-view` → built-in `view` RoleBinding in `default` — read-everything except secrets; `get pods -n default` → yes, `get nodes` → Forbidden). Sign out of any prior Rauthy SSO session first — **the browser session silently completes the code flow as whoever is logged in** (the cache then replays that identity for every kubeconfig against the same issuer+client) |

**Landmine — kubelogin's token cache is keyed by issuer+client-id, NOT user** (cost 2 debugging rounds, cmdshift/platform#91): `~/.kube/cache/oidc-login/` holds one entry per issuer+client, so the `admin` and `viewer` identities share it — the first fresh login wins and the cache replays that token (even a "second" get-token call returns it without any browser flow). Fixes: delete `~/.kube/cache/oidc-login/` between identities, or mint per-identity tokens with `--token-cache-dir` (a **kubelogin** flag — it belongs in the kubeconfig's `exec.args`, not on `kubectl`; kubectl does not forward it) — and remember the SSO cookie on `auth.cloud.test` still decides who the browser flow logs in as (clear that site's cookies / sign out to switch users). Tokens expire after `access_token_lifetime` (1h in clients.json) and a refresh token does NOT survive an auth-container recreate (bootstrap is first-boot-only, container-layer data) — after any `terraform apply -target=module.auth`, re-login in the browser.
...
`just cluster apply` brings up the out-of-cluster companions too, including the **pull-through image cache** (`registry-cloud-test`, terraform module `cluster/local/registry/`). The other companions ride the same pattern: rauthy (`auth-cloud-test`, module `cluster/local/auth/`, `auth.cloud.test`) — the OIDC issuer for the kube-apiserver (cmdshift/platform#154, replacing keycloak), wired via `cluster.apiServer.extraArgs` in `nodes/templates/cluster.tftpl.yaml` and a second kubeconfig `.tmp/kubeconfig-oidc` (kubelogin exec plugin; post-rebuild verification rows above). Its memory sizing and landmines:
...
- **rauthy landmines** (cmdshift/platform#154): the container has **no shell** (`sh` exec fails — distroless-style image); inspect via logs and the CLI binary (`docker exec auth-cloud-test /app/rauthy --help`). Config is `/app/config.toml` (template `auth/templates/rauthy.tftpl.toml`) + `/app/bootstrap/*.json` — **strict parser: an unknown key or wrong type panics at boot**; validate fields against the 0.36 reference config before adding one. Validation rules that cost rounds: confidential client secrets ≥ 64 chars **and alphanumeric-only** (token endpoint rejects others); group ids are internal-allocated (don't set them in groups.json); `groups` must be in the client's `default_scopes` or the claim never rides tokens; token algs pinned `RS256` (EdDSA default is unusable for the kube-apiserver); `bootstrap.admin_email` renames the built-in admin during DB init **before** users.json is read — a users.json entry with the same email dies on the users.email UNIQUE constraint (keep the built-in at admin@localhost, fixture users separate). Sizing 256Mi start-then-audit (keycloak needed 3Gi for the JVM import burst — the entire motivation for the swap)
...
**Landmine — haproxy self-recursion on an unmatched Host: self-flood to OOM in seconds** (cmdshift/platform#94). The shared backend routes by Host header via `set-dst var(txn.host),map(hosts.map)` with `server clear 0.0.0.0:0` — **`set-dst` no-ops when the map misses, leaving dst = haproxy itself**, so each request re-enters the frontend and recurses. Hit live: a talos-backup PUT with `Host=backups.s3.cloud.test` (virtual-host S3 spelling from an old image tag) self-flooded ~25k established conns, pegged haproxy at 400-600% CPU, and OOM-killed the container (exit 137, load 41 on the host, every `*.cloud.test` route down). Fingerprint: self-sourced ESTABLISHED conns in `docker exec cloud-test netstat -tan` (`10.0.128.1:80 → 10.0.128.1:80`) + `Host=<unmatched>` entries in `docker logs cloud-test`. Defense-in-depth now in the template: `http-request deny 403 if !{ var(txn.host),map(...) -m found }` — a map miss fails fast instead of looping (a 4xx, so clients don't retry); a new `*.cloud.test` name goes into the `hosts` map in `cluster/local/main.tf`, never bypass the guard.
