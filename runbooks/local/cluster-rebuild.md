...
- **The wedge can outlive container-level recovery** (seen on `local-test` 80/443 after the cmdshift/platform#70 rebuild): `docker restart`, a terraform-recreate of the container, and a `docker network disconnect/connect` cycle each re-registered the binding (`docker port` correct, host TCP accepts, request bytes sent) but the forward stayed dead — while the SAME `com.docker.backend` process served healthy forwards for other containers (6443 answered 401 throughout). Escalation ladder: container restart → terraform recreate → network reconnect → **Docker Desktop restart** (the only rung not yet exercised — the wedge was left pending, deferred by the operator)
...
| Ingress | `curl -s -o /dev/null -w '%{http_code}' http://local.test` | **404** — correct wiring with zero HTTPRoutes (`server: envoy` header proves the Gateway path; 503 = haproxy backends down). https://local.test same story (TLS passthrough to the Gateway) |
